        ...
        <div class="col-md-8">
            <div class="card" style="background-color: #f8f9fa; border-radius: 15px;">
                <div class="card-header bg-primary text-white">
                    <h3 class="text-center">Editar Donación</h3>
                </div>
                <div class="card-body">
                ...
                <!-- Mensaje de éxito -->
                    ...
                    <form id="updateForm" action="<?= site_url('donaciones/update/' . $donacion->idDonaciones) ?>" method="post">
                     <!-- nuevo donante -->
                    ...
                    <!-- Datos de la personas  AQUI VAMOS-->
                      <div class="form-group">
                            ...
                            <?php foreach ($responsables as $responsable) : ?>
                                   <option value="<?= $responsable->idDatosResponsable ?>" class="<?= $responsable->tipoAsociado ?>"  <?= ($responsable->idDatosResponsable == $donacion->idDatosResponsable) ? 'selected' : '' ?> > <?= $responsable->nombreCompleto ?></option>
                                   <?php endforeach; ?>
                         ...
                         <input type="number" class="form-control" id="cantidad" name="cantidad" value="<?= $donacion->cantidad ?>"  oninput="validarInput(event)" required>
                         </div>
                         <div class="form-group col-md-6">
                         <label for="nombre_completo" style="color: #000;"><i class="fas fa-hands-holding-circle"></i> Proyecto a asignar</label>
                            <select class="form-control" id="idProyecto" name="idProyecto" required>
                            <?php foreach ($proyectos as $proyecto) : ?>
                                    <option value="<?= $proyecto->idProyectos ?>"  <?= ($proyecto->idProyectos == $donacion->idProyectos) ? 'selected' : '' ?> > <?= $proyecto->nombreProyecto ?></option>
                                <?php endforeach; ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="cantidadLetras" name="cantidadLetras" value="<?= $donacion->cantidadLetras ?>"  readonly required>
                        </div>

                        <div class="form-group">
                            <label for="nombre_completo" style="color: #000;"><i class="fa-solid fa-pen-to-square"></i> Concepto</label>
                            <input type="text" class="form-control" id="descripcion" name="descripcion" value="<?= $donacion->descripcion ?>"  required>
                        </div>
                        <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="nombre_completo" style="color: #000;"> <i class="fa-solid fa-calendar-days"></i> Fecha de Donación</label>
                                    <input type="date" class="form-control" id="fecha" name="fecha" value="<?= $donacion->fechaDonacion ?>" required>
                                    </div>
                                    <div class="form-group col-md-6">
                                    <label for="estado" style="color: #000;"><i class="fas fa-check-circle"></i> Estado</label>
                                    <select class="form-control" id="estado" name="estado" required>
                                        <option value="Activo" <?= ($donacion->estado == 'Activo') ? 'selected' : '' ?>>Activo</option>
                                        <option value="Inactivo" <?= ($donacion->estado == 'Inactivo') ? 'selected' : '' ?>>Inactivo</option>
                                    </select>
                                    </div>
                        </div>
